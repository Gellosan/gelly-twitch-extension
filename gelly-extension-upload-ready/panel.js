...
function playAnim(cls) {
  ...
  // clear all action classes first
...
function updateGellyImage(stage, color) {
  ...
  if (img) {
    img.src = src;
    return; // keep overlays; don't nuke container
  }

  // first render — create it
...
async function equipItem(itemId, equipped) {
    try {
        ...
        if (data.success) {
            ...
            console.warn("Equip failed:", data.message);
        }
    } catch (err) {
...
async function renderStore(items) {
    ...
    items.forEach(item => {
        ...
        if (ownedItems.includes(item.id)) {
            // Already owned → disable & grey out
            buyBtn.textContent = "Owned";
            buyBtn.disabled = true;
            buyBtn.style.backgroundColor = "#aaa"; // grey
            buyBtn.style.cursor = "not-allowed";
        } else {
            // Not owned → normal buy logic
...
function updateUIFromState(state) {
  ...
  fetchInventory(twitchUserId).then((data) => {
    renderInventory(data.inventory || []);
  });
}
...
function renderEquippedAccessories(inventory) {
  ...
  // Add currently equipped overlays
...
document.querySelectorAll(".color-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    triggerColorChangeEffect();
    updateGellyImage(currentStage, currentColor);
    // overlays remain; no need to re-render unless your art needs it
  });
});

function getGellyImg() {
  return document.getElementById("gelly-image");
}
